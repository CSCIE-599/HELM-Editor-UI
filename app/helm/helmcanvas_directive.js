...
.directive('helmCanvas', function() {
 ...
 return {
  	...
  	//
  	// Controller for the helmCanvas directive.
  	...
  	//
  	controller: 'HelmNotationController',
  };
})
	...
	return {
		...
		link: function (scope, elem, attr) {
			...
			$(elem).bind("input propertychange", function () {
				...
				scope.viewModel = new helmnotation.ChartViewModel(dataModel);
...
// (which is possible, just not ideal).
//
.controller('HelmNotationController', ['$scope', 'dragging', '$element', function HelmNotationController ($scope, dragging, $element) {
	...
	$scope.connectorMouseDown = function (evt, node, connector, connectorIndex, isInputConnector) {
		...
		dragging.startDrag(evt, {
			...
			dragStarted: function (x, y) {
				...
				$scope.dragPoint1 = helmnotation.computeConnectorPos(node, connectorIndex, isInputConnector);
				$scope.dragPoint2 = {
					x: curCoords.x,
					y: curCoords.y
				};
				$scope.dragTangent1 = helmnotation.computeConnectionSourceTangent($scope.dragPoint1, $scope.dragPoint2);
				$scope.dragTangent2 = helmnotation.computeConnectionDestTangent($scope.dragPoint1, $scope.dragPoint2);
			},
